            ...
            if (editor.options.imagePopup) {
                ...
                editor.addListener('selectionchange', function (t, causeByUi) {
                    ...
                    if (editor.ui._dialogs.linkDialog) {
                        ...
                        if(!link) return;
                        var url;
                        var _href = link.getAttribute('_href') || link.getAttribute('href',2);
                        var miniprogramAppid = link.getAttribute('data-miniprogram-appid')
                        var miniprogramNickname = link.getAttribute('data-miniprogram-nickname')
                        if (link && (_href||miniprogramAppid)) {
                            url = _href
                            var txt = url;
                            if (url.length > 30) {
                            ...
                            if(miniprogramAppid){
                                txt = miniprogramNickname||'小程序'
                            }
                            if (html) {
                                html += '<div style="height:5px;"></div>'
                            }
                            html += popup.formatHtml(
                                '<nobr>' + '<a target="_blank" class="_linkstyle" href="' + url + '" title="' + url + '" >' + txt + '</a>' +
                                    '<span class="edui-clickable lr-border _addhover" onclick="$$._onRemoveButtonClick(\'unlink\');"> ' + editor.getLang("clear")  + '</span>' +
                                    ' <span class="edui-clickable _addhover" onclick="$$._onEditButtonClick();">' + editor.getLang("modify") + '</span></nobr>');
